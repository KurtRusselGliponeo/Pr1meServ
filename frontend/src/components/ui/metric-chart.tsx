...
export function BarMetricChart({
  ...
  return (
    <div className={cn('space-y-4', className)}>
      <div className="grid h-56 grid-cols-[repeat(auto-fit,minmax(40px,1fr))] items-end gap-3 rounded-3xl border border-border/70 bg-muted/20 p-4">
        {data.map((item) => {
    ...
    <div className={cn('space-y-4', className)}>
      <div className="rounded-3xl border border-border/70 bg-muted/20 p-4">
        ...
        <div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(72px,1fr))] gap-2">
          {data.map((item) => (
            <div key={item.label} className="rounded-2xl bg-background/90 px-3 py-2 text-center">
