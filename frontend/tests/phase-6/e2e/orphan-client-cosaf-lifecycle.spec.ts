...
async function attachWorkflowMocks(
  ...
  await context.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/documents/presigned-url') && method === 'POST') {
      await fulfillJson(route, {
        documentId: '77777777-7777-4777-8777-777777777777',
        signedUrl: 'https://storage.local/mock-upload',
      });
      return;
    }
...
test('simulates admin reassignment, agent upload, and manager approval in a headless browser', async ({
  browser,
}) => {
  ...
  await expect(
    adminPage.getByRole('heading', { name: /admin reassignment screen/i }),
  ).toBeVisible();
  await adminPage.getByRole('button', { name: /alex source/i }).click();
  await adminPage.getByRole('button', { name: /jamie agent/i }).click();
  await adminPage.getByRole('button', { name: /^add$/i }).click();
  await adminPage.getByRole('button', { name: /run preflight and review/i }).click();
  await adminPage.getByRole('button', { name: /reassign clients/i }).click();
  await expect(adminPage.getByText(/1 client record\(s\) were reassigned\./i)).toBeVisible();
  ...
  await expect(agentPage.getByRole('heading', { name: /agent upload portal/i })).toBeVisible();
  await agentPage.getByRole('button', { name: /taylor santos/i }).click();
  await agentPage
    .locator('input[type="file"]')
    .setInputFiles({
      name: 'cosaf.pdf',
      mimeType: 'application/pdf',
      buffer: Buffer.from('mock cosaf'),
    });
  await agentPage.getByRole('button', { name: /upload and create approval/i }).click();
  await expect(agentPage.getByText(/manager approval queue has been updated/i)).toBeVisible();
  ...
  await expect(
    managerPage.getByRole('heading', { name: /manager cosaf approvals/i }),
  ).toBeVisible();
  await managerPage.getByRole('button', { name: /^return$/i }).click();
  await expect(managerPage.getByRole('button', { name: /return to agent/i })).toBeDisabled();
  await managerPage.getByLabel(/rejection reason/i).fill('Missing branch signature page.');
  await expect(managerPage.getByRole('button', { name: /return to agent/i })).toBeEnabled();
  await managerPage.getByRole('button', { name: /cancel/i }).click();
  await managerPage.getByRole('button', { name: /^approve$/i }).click();
  await expect(managerPage.getByText(/approval queue is clear/i)).toBeVisible();
  await managerContext.close();

  expect(state.approvals[0]?.status).toBe('APPROVED');
  expect(state.clients[0]?.caseStatus).toBe('BM Signed');
});
