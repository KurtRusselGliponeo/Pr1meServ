...
async function seed() {
  ...
  // 1.5. Upsert BM Agent Profile
  const bmDisplayName = `${BM.firstName} ${BM.lastName}`;
  const bmAgentCode = `BM-${BRANCH_CODE}`;

  const [existingBmAP] = await db.select({ id: agentProfiles.id }).from(agentProfiles)
    .where(eq(agentProfiles.userId, bmUserId)).limit(1);

  if (existingBmAP) {
    await db.update(agentProfiles).set({
      agentCode: bmAgentCode,
      branchCode: BRANCH_CODE,
      displayName: bmDisplayName,
      status: 'Active',
      updatedAt: new Date(),
    }).where(eq(agentProfiles.id, existingBmAP.id));
    console.log(`  ↩  BM Agent Profile refreshed`);
  } else {
    ...
    console.log(`  ✓  BM Agent Profile created`);
  }
  ...
  console.log('\n  Seeding PER records...');
  const persistencyValues = [85.32, 78.91, 92.14, 88.67, 73.45, 81.56];
  const unitPer = 83.90;
  const branchPer = 83.70;

  if (persistencyValues.length !== AGENTS.length) {
    throw new Error(
      `PER seed configuration mismatch: expected ${AGENTS.length} persistency values, received ${persistencyValues.length}.`,
    );
  }
